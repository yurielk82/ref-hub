...
export const AX_HERO = {
  ...
  summary:
    '영업관리 현장에서 반복 정산, 데이터 검증, 리포트 작성의 병목을 직접 겪었고, 업무 외 시간까지 활용해 AI 기반 자동화와 데이터 시스템을 자발적으로 설계·검증했습니다. 같은 접근을 제약 영업관리에서 시작해 엔지니어링 시험·팀 진단·사내 복지 같은 다른 도메인과, 그 작업을 굴리는 자기 운영 도구에까지 적용했습니다 — 사람이 검수할 지점과 시스템이 처리할 지점을 나누는 AX 실험으로.',
  evidence: [
    {
      label: '단독 운영',
...
export const AX_CASE_STUDIES: AxCaseStudy[] = [
  ...
  },
  {
    ...
    intervention:
      '낙인을 어떻게 없애느냐가 전부라, 세 가지를 박았습니다. ① 방어기제(Vaillant)부터 의사소통 유형(사티어)·성향(Big Five)까지 협업에 도움이 되는 기법을 쓰되, 문항을 추상적 성격이 아니라 일터 행동("새 협업 툴 시도", "팀 갈등 상황")으로 한정해 평가가 아닌 협업 맥락임을 드러냈습니다. ② 노출을 계층화 — 개인 결과는 RLS로 본인만, 팀 집계는 최소 3명이 응답하기 전엔 개인 역산이 불가능하도록 구간으로만 보이고 소유자 화이트리스트만 접근하게 했습니다. ③ 동의 모달·결과 고지·쎄지 않은 부드러운 말투와 용어("진단" 대신 "협업 카테고리 추정")로 "HR·임상 평가가 아니다"를 일관되게 반복했습니다.',
    outcome:
...
export const AX_GROUNDING: AxGrounding = {
  ...
  summary:
    'AI는 모르는 것도 그럴듯하게 지어냅니다(환각). 그래서 모델을 잘 타이르는 대신, 사실을 실제 소스에 묶어 애초에 지어낼 수 없는 환경을 설계합니다 — 하네스 엔지니어링. 특정 프로젝트가 아니라 챗봇·대시보드 등 AI를 쓰는 모든 화면에 적용하는 방식입니다.',
  examples: [
    {
      label: '법령',
      text: '제약은 약사법·고시 같은 법령이 의사결정에 직결됩니다. 그냥 물으면 없는 조문을 지어내므로, 한국 법령 데이터(법령 MCP)를 직접 물려 실제 원문·사례만 인용하게 했습니다.',
    },
    {
      label: '숫자·금액',
