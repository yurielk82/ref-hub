...
export const AX_HERO = {
  ...
  operatingModel: {
    label: '운영 방식',
    text: '보통은 AI를 노트북 옆 조수처럼 쓰지만, 저는 서비스가 실제로 도는 OCI 서버에 Claude Code를 상주시켜 어디서든 SSH로 구현·검증·배포를 그 자리에서 끝냅니다. Cloudflare·Nginx·systemd·Docker 위에서 self-host Supabase를 포함한 프로덕션 서비스 20여 개를 단독 운영하며, 모든 변경은 빌드 검증·헬스체크·실패 시 자동 롤백 게이트를 통과해야 라이브가 됩니다.',
  },
  focus: [
    '현업 병목 진단',
    'ERP·운영 데이터 정리',
