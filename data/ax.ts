...
export const AX_CASE_STUDIES: AxCaseStudy[] = [
  ...
  },
  {
    ...
    outcome:
      '사진 한 장씩 눈으로 보던 매달 15일짜리 수작업을, 4단계 위변조 방어가 1차로 걸러내고 사람은 의심 건만 검수하는 증거 기반 워크플로우로 재설계했습니다.',
    disclosure: 'range',
    evidenceLabel: '프로젝트 상세',
  },
  {
  ...
  {
    projectSlug: 'srt',
    label: 'Concurrency-Safe Rate-Limit Engineering',
    problem:
      '인기 SRT 노선은 판매 직후 몇 초 만에 매진이라 수동으로는 못 잡고, 서버 한 IP에서 여러 사용자가 동시에 자동 조회하면 SRT의 속도 제한·봇 탐지에 IP가 통째로 차단됩니다.',
    intervention:
      '상충하는 요구(속도 vs 차단 회피 vs 비용)를 동시성 규칙으로 풀었습니다. ① 모든 요청을 Redis 전역 FIFO 큐에 모아 최소 1초 간격을 강제해 차단을 회피하고, ② 동시에 도는 매크로가 있을 때만 sticky 프록시를 쓰고 단독이면 직결해 프록시 비용·지연을 아끼며, ③ 50회마다 세션을 갱신하고 0.1~0.5초 랜덤 간격으로 탐지 패턴을 흩뜨리고, ④ 동시 6개 + 대기열 5개로 공정성과 한도를 동시에 지켰습니다.',
    outcome:
      '"빠르게, 그러나 차단되지 않게, 비싸지 않게"라는 상충 조건을 구체적 동시성 규칙으로 풀어, 다중 사용자가 실제로 쓰는 예매 자동화 서비스로 운영 중입니다.',
    disclosure: 'public',
    evidenceLabel: '프로젝트 상세',
  },
]
