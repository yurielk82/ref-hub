...
export const AX_CASE_STUDIES: AxCaseStudy[] = [
  ...
  },
  {
    ...
    label: 'Multi-API Sync + Learning Pipeline',
    problem:
      'ERP에서 내려온 의약품 공급내역에는 표준코드 불일치·오류, 공급규격 불일치, 그리고 규격에서 파생되는 수량·단가 불일치가 섞여 있어, 담당자가 표준코드 마스터와 일일이 대조하며 한 행씩 손으로 바로잡아야 했습니다.',
    intervention:
      '의약품관리종합정보센터(KPIS) 연동을 SOAP·JWT·REST·WSDL 네 가지 방식으로 붙여 표준코드 마스터(포장수량·상한금액 등)를 끌어오고, 코드매핑 → 마스터매칭 → 단가 자동수정 → 규격보정 → 규칙엔진 → 집계로 이어지는 단계별 자동 보정 파이프라인을 만들었습니다. 규격을 마스터 포장수량으로 교정하면 그에 맞춰 수량(공급수량 × 규격비)과 단가(공급금액 ÷ 수량)를 함께 다시 계산하고, 표준코드별 단가 비율을 학습해 반복되는 오류를 자동으로 채웁니다.',
    outcome:
      '한 행씩 눈으로 대조하던 수동 검증을 표준코드 마스터·학습 규칙 기반의 단계별 자동 보정 흐름으로 옮기고, 웹에서 검증된 제출 가능 데이터를 API로 바로 신고하는 데까지 연결했습니다.',
    disclosure: 'withheld',
    evidenceLabel: '프로젝트 상세',
  },
  {
